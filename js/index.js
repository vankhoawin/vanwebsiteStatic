/* async.js
 *
 *  need this library for running
 *  the multiple Flickr queries in series
 */
...
$(document).ready(function () {
  ...
  var ALBUM_QUALITY   = 4                                  // quality of album thumbnails
    , PHOTO_QUALITY   = 8                                  // quality of album photos
    , USER_ID         = '126052905@N03'                    // Van's Flickr ID
    , API_KEY         = 'aa97f39fc7ff944178ebd92711b9ab35' // dummy Flickr account API key
    , albumList       = {}                                 // hold albums for navigating back
  ...
  function processAlbum(album, callback) {
    $.ajax({
      ...
      callback(null, {
        ...
        thumbnail: data.sizes.size[ALBUM_QUALITY].source,
      });
    }).error(function (err) {
      console.error(err);
    });
  ...
  function addPhotosToDOM(err, albums) {
    ...
    for (var i = 0; i < albums.length; ++i) {
      ...
      photoHTML += (
        ...
        '    <a data-album-id="' + album.id + '" href="javascript:void(0)">' +
        '      <img src="' + album.thumbnail + '" />' +
        '    </a>' +
        '  </div>' + 
        '  <div class="section-description">' +
        '    <a data-album-id="' + album.id + '" href="javascript:void(0)">' + album.title + '</a>' +
  ...
  function getSingleAlbum(e) {
    ...
    $photoData = $(
      '<div id="album-caption">' +
      '  <a id="navigate-photo-home" href="javascript:void(0)">Photography</a>' +
      '  <span> / </span>' +
      '  <h3 data-photo-id="' + album.id + '">' + album.title + '</h3>' +
      '  <i>' + album.size + ' ' + (album.size > 1 ? 'photos' : 'photo') + '</i>' +
      '  <p>' + album.description + '</p>' +
      '</div>' + 
  ...
  function loadSingleAlbumPhoto(photo, callback) {
    $.ajax({
      ...
      callback(null, {
        id: photo.id,
        thumbnail: data.sizes.size[PHOTO_QUALITY].source,
      });
    }).error(function (err) {
      console.error(err);
    });
